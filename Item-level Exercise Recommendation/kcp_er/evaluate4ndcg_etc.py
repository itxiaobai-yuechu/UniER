...
def preprocess_test_data(test_data):
    ...
    for i in range(test_data.shape[0]):
        uid = normalize_uid(test_data.iloc[i]['uid'] if 'uid' in test_data.columns else i)
...
if __name__ == '__main__':
    ...
    test_file = f'{data_dir}/test_sequences.csv'
    REL_generator_file = f'{data_dir}/REL_generator_result_{REL_n}.txt'
    Q_matrix = np.load(f'{data_dir}/Q.npy')
    ...
    with open(REL_generator_file, 'r') as f:
        for line in f:
            ...
            stu_rec_ex[normalize_uid(stu_id)] = rec_ex
