...
def distance(L):
    ...
    L_norm = np.divide(L, norm, out=np.zeros_like(L, dtype=float), where=norm != 0)
...
if __name__ == '__main__':
    ...
    ES = []
    with open(f'{data_dir}/EB_filter_{N}.txt', 'r') as f:
        for line in f:
    ...
    with open(f'{data_dir}/REL_generator_result_{REL_n}.txt', 'w') as f:
