...
class ProximityCalculator:
    ...
    def load_assist_data(self, csv_path):
        ...
        uid_values = df['uid'] if 'uid' in df.columns else df.index
        self.uid_to_row = {str(uid): row_idx for row_idx, uid in enumerate(uid_values)}
        self.uid_to_row.update({f'uid{uid}': row_idx for row_idx, uid in enumerate(uid_values)})
    ...
    def load_recommend_dict(self, rec_path):
        ...
        with open(rec_path, 'r', encoding='utf-8') as f:
            for row_idx, line in enumerate(f):
                ...
                source_uid = parts[0]
                if source_uid not in self.uid_to_row:
                    continue
                rec_dict[self.uid_to_row[source_uid]] = prob_list
        return rec_dict
...
if __name__ == "__main__":
    ...
    csv_file = f"{data_dir}/test_sequences.csv"
    rec_file = f"{data_dir}/recommendation_20.txt"
