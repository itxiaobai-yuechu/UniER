...
def EB_filter(Q, pkm, pkc, dielta=0.7, N=150):
    ...
    for i in tqdm(range(student_num)):
        ...
        while cnt < N:
            ...
            cnt += 1
            is_select.append(j)
            cos_sim = np.dot(pkc_i, Q[j]) / (np.linalg.norm(pkc_i) * np.linalg.norm(Q[j]))
...
if __name__ == '__main__':
    ...
    pkc_300 = torch.load(f'datasets/{dataset}/select/pkc.pth')
    pkc_300_new = get_pkc(test_raw, pkc_300, kc_num=kc_num[dataset])
    torch.save(pkc_300_new, f'datasets/{dataset}/select/pkc_final.pth')

    pkm = torch.load(f'datasets/{dataset}/select/pkm.pth')
    pkc = torch.load(f'datasets/{dataset}/select/pkc_final.pth')
    ...
    with open(f'datasets/{dataset}/select/EB_filter_{N}.txt', 'w') as f:
