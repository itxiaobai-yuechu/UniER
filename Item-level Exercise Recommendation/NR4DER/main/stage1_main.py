...
def parse_args():
    ...
    parser.add_argument('--is_pkc', action='store_true', help='get pkc instead of pkm')
    mlstm_group = parser.add_mutually_exclusive_group()
    mlstm_group.add_argument('--is_mlstm', dest='is_mlstm', action='store_true', help='use MLSTM')
    mlstm_group.add_argument('--no_mlstm', dest='is_mlstm', action='store_false', help='use LSTM')
    parser.set_defaults(is_mlstm=True)
    parser.add_argument('--train_data_file', type=str, default=os.path.join(PROJECT_ROOT, 'dataset/data_200/assist2017/train_valid_sequences.csv'),
                         help='the path of train data')
    parser.add_argument('--test_data_file', type=str, default=os.path.join(PROJECT_ROOT, 'dataset/data_200/assist2017/test_sequences.csv'),
                         help='the path of test data')
    parser.add_argument('--Q_file', type=str, default=os.path.join(PROJECT_ROOT, 'dataset/data_200/assist2017/Q.npy'),
                         help='the path of test data')
    parser.add_argument('--stu_ks_save_file', type=str, default=os.path.join(PROJECT_ROOT, 'stu_ks_save/assist2017_200'), help='student ks save file')
    parser.add_argument('--plot_dir', type=str, default=None)
    parser.add_argument('--show_plots', action='store_true',
                        help='also open interactive plot windows after saving figures')
...
def plot_results(indicator_result, plot_dir, show_plots=False):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(plot_dir, 'test_performance.png'), dpi=150)
    if show_plots:
        plt.show()
    plt.close()
...
def main(args):
    ...
    data_dir = os.path.join(PROJECT_ROOT, 'dataset', 'data_200', dataset)
    args.train_data_file = os.path.join(data_dir, 'train_valid_sequences.csv')
    args.test_data_file = os.path.join(data_dir, 'test_sequences.csv')
    args.Q_file = os.path.join(data_dir, 'Q.npy')
    args.stu_ks_save_file = os.path.join(PROJECT_ROOT, 'stu_ks_save', f'{dataset}_200')
    ...
    Q_file = args.Q_file
    stu_ks_save_file = args.stu_ks_save_file
    ...
    plot_dir = args.plot_dir or os.path.join(PROJECT_ROOT, 'outputs', dataset)
    plot_results(indicator_result, plot_dir, args.show_plots)
