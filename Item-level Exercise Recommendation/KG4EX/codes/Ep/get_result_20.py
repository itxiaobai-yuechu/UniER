...
save_dir = f'../data/{dataset}/Test20/'
os.makedirs(save_dir, exist_ok=True)
file_path = os.path.join(save_dir, "recommend_20_new.txt")

test_df = pd.read_csv(test_data_path)
test_uid_list = test_df["uid"].unique().tolist()

with open(file_path, "w", encoding="utf-8") as f:
    for student_id in test_uid_list:
        if isinstance(student_id, str) and student_id.startswith("uid"):
            std_id = int(student_id[3:])
        else:
            std_id = int(student_id)
            
        if std_id in rec_Q:
            top20_ex = rec_Q[std_id][:20]
        else:
            top20_ex = []
        
        ex_str = ",".join(map(str, top20_ex))
        f.write(f"{std_id}\t{ex_str}\n")
