...
def ranked_exercise_ids(scores, n):
    """Rank valid scores while preserving their original qid positions."""
    ranked = sorted(
        ((qid, score) for qid, score in enumerate(scores) if score is not None),
        key=lambda item: item[1], reverse=True
    )
    return [qid for qid, _ in ranked[:n]]

def ACC(uid_mlkc_dict, uid_ex_scores, Q, r1, n):
    acc = []
    for item in uid_ex_scores:
        uid, scores = item[0], item[1]
        uid_ex_score = ranked_exercise_ids(scores, n)
        if not uid_ex_score:
            continue
        ...
        acc.append(diff / len(uid_ex_score))
...
def NOV(uid_kc_response, uid_ex_scores, Q, n):
    ...
    for item in uid_ex_scores:
        ...
        for ex_id in uid_ex_score:
            ...
            jaccard_similarity += 1 - intersection / union if union else 0.0
        jaccsim.append(jaccard_similarity / len(uid_ex_score))
    return (np.mean(jaccsim), np.std(jaccsim)) if jaccsim else (0.0, 0.0)
