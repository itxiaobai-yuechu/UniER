...
def novlty(test_raw, Q, rec_Q):
    nov = []
    kc_responses = test_raw[['concepts', 'responses', 'uid']]
    for i in range(len(kc_responses)):
        uid = kc_responses.iloc[i]['uid']
        if uid not in rec_Q:
            continue
        kc_res_yes_set = set()
...
if __name__ == '__main__':
    ...
    test_raw = pd.read_csv(f'{dataset}/test_sequences.csv')
    pkm = torch.load(f'{dataset}/pkm.pth',weights_only=True)
    Q = np.load(f'{dataset}/Q.npy')
    pkm = pkm.to('cpu').numpy()
    rec_Q = {}
    with open(f'../fin_res/{dataset}/recommend.txt', 'r') as f:
        for line in f:
            line = line.strip().split('\t')
            rec_Q[int(line[0])] = np.array([int(q) for q in line[1].split(',')])
