...
class ProximityCalculator:
    ...
    def load_recommend_dict(self, rec_path):
        ...
        with open(rec_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                if not line:
                    continue
                uid_str, prob_str = line.split('\t')
                uid = int(uid_str)
                prob_list = list(map(int, prob_str.split(',')))
                rec_dict[uid] = prob_list
        return rec_dict
    ...
    def calculate_final_proximity(self, csv_path, rec_path):
        ...
        valid_uids = [uid for uid in rec_dict if uid < self.total_csv_users]
...
if __name__ == "__main__":
    ...
    csv_file = "../../data/ASSISTments2009/assist2009.csv"
    rec_file = "./recommend.txt"
    
    mean_prox, std_prox = calc.calculate_final_proximity(csv_file, rec_file)
    print(f"Proximity mean: {mean_prox:.6f}")
    print(f"Proximity standard deviation (std): {std_prox:.6f}")