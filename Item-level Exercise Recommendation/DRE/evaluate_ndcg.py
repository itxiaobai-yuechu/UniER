...
if __name__ == '__main__':
    ...
    REL_generator_file = f'./data/{dataset}/recommended_problems.txt'
    Q_matrix = np.load(f'./data/{dataset}/Q.npy')
    ...
    for k in [1, 3, 5, 10]:
        hit, ndcg, map_score, mrr, precision, f1, recall, div, valid_test_stu_num = calculate_metrics(
            stu_true_response, stu_rec_weak_kc, stu_rec_ex, Q_matrix, k=k
        )
        print(
            f'k = {k}, ndcg: {ndcg:.3f}, map: {map_score:.3f}, mrr: {mrr:.3f}, '
            f'precision: {precision:.3f}, f1: {f1:.3f}, recall: {recall:.3f}, '
            f'div: {div:.3f}, valid_test_stu_num: {valid_test_stu_num}'
        )
