...
def calculate_all_Ep(dataset, num_concepts, Q_matrix_path, test_csv_path, rerank_txt_path, device):
    ...
    for i in range(len(test_df)):
        ...
        rec_str = rerank_lines[i].strip().split("\t")[-1]
        rec_questions = [int(q) for q in rec_str.split(",") if q]
        ...
        for k in test_ks:
            ...
            if (full_score - initial_score) != 0:
                ...
                Ep = 0.0
            print(f"k:{k}, Student {i}: Initial Score = {initial_score}, Final Score = {final_score}, Ep = {Ep:.4f}")
...
def calculate_all_Ep_KTmodel(dataset, num_concepts, test_csv_path, rerank_txt_path, device):
    ...
    for i in range(len(test_df)):
        ...
        rec_str = rerank_lines[i].strip().split("\t")[-1]
        rec_concepts = [int(q) for q in rec_str.split(",") if q]
    ...
    print("\n" + "="*50)
    print(f"Dataset: {dataset} | Full Knowledge Component Ep Test Recommendation Lengths 5/10/20")
    print("="*50)
    for k in test_ks:
        valid_num = result_dict[k]["valid_num"]
        mean_ep = result_dict[k]["ep"] / valid_num if valid_num > 0 else 0.0
...
if __name__ == "__main__":
    ...
    if model == 'NR4DER':
        Q_matrix_path = f"melt-lstm/dataset/data_200/{dataset}/Q.npy"
        test_csv_path = f"melt-lstm/dataset/data_200/{dataset}/test_sequences.csv"
        rerank_txt_path = f"melt-lstm/dataset/data_200/{dataset}/Reranking_result_melt_20.txt"
        calculate_all_Ep(dataset, num_concepts, Q_matrix_path, test_csv_path, rerank_txt_path, device)
    elif model == 'DRE':
        ...
        Q_matrix_path = f"./kcp_er/datasets/{dataset}/select/Q.npy"
        test_csv_path = f"./kcp_er/datasets/{dataset}/select/test_sequences.csv"
        rerank_txt_path = f"./kcp_er/datasets/{dataset}/select/recommendation_20.txt"  
        calculate_all_Ep(dataset, num_concepts, Q_matrix_path, test_csv_path, rerank_txt_path, device)
        ...
        test_csv_path = f"./KT_Base_Rec/dataset/{dataset}/test_sequences.csv"
