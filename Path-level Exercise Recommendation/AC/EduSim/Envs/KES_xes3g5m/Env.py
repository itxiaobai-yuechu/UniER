...
import numpy as np
from EduSim.Envs.KES_xes3g5m.meta.Learner import LearnerGroup, Learner
from EduSim.Envs.shared.KSS_KES import episode_reward
from EduSim.spaces import ListSpace
from EduSim.Envs.deep_model import KTnet
...
__all__ = ["KESxes3g5mEnv"]


class KESxes3g5mEnv(Env):
    def __init__(self, dataRec_path, seed=None, cudaDevice=None):
        super().__init__()
        self.target_type = 'portion'
        self.type = 'KES'
        self.env_name = 'KESxes3g5m'
        self.random_state = np.random.RandomState(seed)
        self.graph_embeddings = get_graph_embeddings('KES_xes3g5m')
        self.dataRec_path = dataRec_path

        self.knowledge_structure = nx.DiGraph()
        with open(f"{get_proj_path()}/data/dataProcess/xes3g5m/nxgraph.pkl", "rb") as file:
        ...
        self.KTnet = KTnet(input_size=self.feature_dim,
                             emb_dim=self.embed_dim,
                             hidden_size=self.hidden_size,
                             num_skills=self.num_skills,
                             nlayers=2).to(self.device)
        directory = f'{get_proj_path()}/EduSim/Envs/KES_xes3g5m/meta_data'
    ...
    def _exam(self, learner: Learner, detailed=False, reduce="sum"):
        state = learner.state
        knowledge_response = {}
    ...
    def update_learner_state(self):
        ...
        with torch.no_grad():
            self._learner._state = torch.sigmoid(self.KTnet(input_data).permute(1, 0, 2).squeeze(0)[sequence_length - 1])
