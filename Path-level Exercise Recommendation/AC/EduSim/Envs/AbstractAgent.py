...
class AbstractAgent:
    def __init__(self, env, args):
        ...
        if self.args.get('load_model_test', False):
            if os.path.exists(f'{get_proj_path()}/EduSim/SavedAgents/{self.agent.name}_{self.env.env_name}_{self.args["max_steps"]}_{self.args["max_episode_num"]}_{self.env.target_type}.pth'):
                checkpoint = torch.load(f'{get_proj_path()}/EduSim/SavedAgents/{self.agent.name}_{self.env.env_name}_{self.args["max_steps"]}_{self.args["max_episode_num"]}_{self.env.target_type}.pth')
            else:
    ...
    def begin_episode(self, learner_profile):
        ...
        if self.args.get('know_all_log', False):
    ...
    def step(self):
        ...
        if self.args.get('Repe_control', False):
    ...
    def end_episode(self, observation, reward, done, info):
        ...
        if self.args['episode_count'] == self.args['max_episode_num'] and not self.args['load_model_test']:
            torch.save({
                ...
                'args': self.args,
            }, f'{get_proj_path()}/EduSim/SavedAgents/{self.agent.name}_{self.env.env_name}_{self.args["max_steps"]}_{self.args["max_episode_num"]}_{self.env.target_type}.pth')
            self.TestAgent()
    ...
    def update(self, transition_dict):
        ...
        if self.KTnet_trian:
            ...
            dkt_loss = compute_dkt_loss(KTnet_states, batch_data_one_hot, device=self.device)
            dkt_loss.backward()
    ...
    def TestAgent(self):
        ...
        test_path = f'{get_proj_path()}/EduSim/SavedAgents/{self.agent.name}_{self.env.env_name}_{self.args["max_steps"]}_{self.args["max_episode_num"]}_{self.env.target_type}_test.txt'
    ...
    def goal_oriented_candidate_choose(self, DKT_states):
        ...
        while len(candidates) == 0:
            ...
            if self.args['cur_goal'] == -1:
                if len(not_achieved_goals) > 0:
                     self.args['cur_goal'] = random.choice(not_achieved_goals)
                else:
                     return [i for i in range(self.action_dim)]
