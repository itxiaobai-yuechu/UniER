...
class AbstractAgent:
        ...
        if self.args['episode_count'] == int(self.args['max_episode_num']) and not self.args['load_model_test']:
            if self.args['agent'] == 'PFD':
                ...
                torch.save({
                    ...
                    'args': self.args,
                }, f'{get_proj_path()}/EduSim/SavedAgents/{self.agent.name}_{base_p_name}_{self.env.env_name}_{self.args["max_steps"]}_{self.args["max_episode_num"]}.pth')
            else:
                torch.save({
                    'model_state_dict': self.agent.state_dict(),
                    'args': self.args,
    ...
    def TestAgent(self):
        ...
        if self.args['agent'] == 'PFD':
            base_p_name = self.args['PFD_base_policy']
            test_path = f'{get_proj_path()}/EduSim/SavedAgents/{self.agent.name}_{base_p_name}_{self.env.env_name}_{self.args["max_steps"]}_{self.args["max_episode_num"]}_test.txt'
