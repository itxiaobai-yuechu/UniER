...
import numpy as np
from EduSim.Envs.KES_ednet.meta.Learner import LearnerGroup, Learner
...
class KESednetEnv(Env):
    def __init__(self, dataRec_path, seed=None, cudaDevice=None):
        super().__init__()
        self.target_type = 'portion'
        self.type = 'KES'
        self.env_name = 'KESednet'
        self.random_state = np.random.RandomState(seed)
        self.graph_embeddings = get_graph_embeddings('KES_ednet')
        self.dataRec_path = dataRec_path

        self.knowledge_structure = nx.DiGraph()
        with open(f"{get_proj_path()}/data/dataProcess/ednet/nxgraph.pkl", "rb") as file:
        ...
        directory = f'{get_proj_path()}/EduSim/Envs/KES_ednet/meta_data'
