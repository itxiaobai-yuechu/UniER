...
class DKThandler(object):
    def __init__(self):
        ...
        self.KTnet = KTnet(input_size=2 * self.num_skills,
                             emb_dim=self.embed_dim,
                             hidden_size=self.hidden_size,
                             num_skills=self.num_skills,
                             nlayers=2).to(self.device)
        self.optimizer = torch.optim.Adam(self.KTnet.parameters(), lr=self.learning_rate)
        self.loss_f = nn.BCEWithLogitsLoss()
        if True:
            print('testing')
            self.KTnet.load_state_dict(torch.load(f'{get_proj_path()}/EduSim/Envs/KSS/meta_data/DKT_USE_DATA/all_data_trained_DKT_model.pth', map_location=self.device)['model'])
            self.test()
            assert 0
    ...
    def train(self):
        ...
        for epoch in range(self.epoch_num):
            for i, batch_data in enumerate(self.train_loader):
                ...
                loss.backward()
                torch.nn.utils.clip_grad_norm_(self.KTnet.parameters(), 20)
                self.optimizer.step()
            ...
            print('auc:' + str(auc))
            print('r2:' + str(r2))
            state = {'model': self.KTnet.state_dict(), 'optimizer': self.optimizer.state_dict(), 'epoch': epoch}
