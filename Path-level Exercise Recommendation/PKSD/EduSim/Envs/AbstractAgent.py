...
class AbstractAgent:
    def __init__(self, env, args):
        ...
        self.graph_embedding_dim = torch.tensor(self.env.graph_embeddings).size(-1)
        self.agg_model = SelfAttention(num_attention_heads=2,
                                       input_size=self.graph_embedding_dim,
                                       hidden_size=self.graph_embedding_dim,
                                       hidden_dropout_prob=0.0,
                                       device=self.device).to(self.device)
        if self.graph_embedding_input:
            self.RL_input_dim = self.RNN_encoder_output_dim + self.graph_embedding_dim

        self.dqn_advance_types = {'dueling dqn': True,
                                  ...
                                  'noisy dqn': False}
        if args['agent'] == 'DQN':
            self.agent = DQNAgent(self.RL_input_dim,
                                  self.action_dim,
                                  self.hidden_size_1,
                                  self.hidden_size_3,
                                  self.dqn_advance_types,
                                  self.target_update,
                                  self.env,
                                  self.learning_rate,
                                  self.gamma,
                                  self.device,
                                  args=args)
        elif args['agent'] in ['AC']:
            self.buffer_size = 1
            self.begin_train_length = 0
            self.epoch_num = 1
            self.agent = ActorCritic(self.RL_input_dim, self.action_dim, self.hidden_size_1, self.hidden_size_3,
                                     self.env, lr_rate=self.learning_rate, gamma=self.gamma, device=self.device, lr_sche=self.lr_sche, args=args)
        elif args['agent'] == 'PPO':
            self.buffer_size = 1
            self.begin_train_length = 0
            self.epoch_num = 1
            ...
            self.agent = CSEAL(self.RL_input_dim,
                               ...
                               device=self.device,
                               args=args)
        elif args['agent'] == 'DKTRec':
            self.buffer_size = 1
            self.begin_train_length = 0
            self.epoch_num = 1
            self.agent = DKTRec(self.DKTnet, max_length=self.max_sequence_length, env=self.env, device=self.device)
            ...
            self.epoch_num = 1
            c2 = 0.001
            self.agent = PFD(self.RL_input_dim,
                             self.action_dim,
                             self.hidden_size_1,
                             self.hidden_size_3,
                             env=env,
        ...
        self.agg_model_optimizer = torch.optim.Adam(self.agg_model.parameters(), lr=self.learning_rate)
        ...
        if args['model_based'] == 'DAS3H':
            if self.args['agent'] not in ['PPO', 'PFD'] or (self.args['agent'] == 'PFD' and self.args['PFD_base_policy'] != 'PPO'):
                raise ValueError('RLtutor model based only work with PPO')
            if self.env.env_name == 'KESassist15':
                innerModel_dataset_path = f'{get_raw_data_path()}/ASSISTments2015/processed/'
            elif self.env.env_name == 'KESjunyi':
                innerModel_dataset_path = f'{get_raw_data_path()}/junyi/data/student_log_kt_None'
            elif self.env.env_name == 'KSS':
                innerModel_dataset_path = f'{get_proj_path()}/EduSim/Envs/KSS/meta_data/exLog.txt'
            elif self.env.env_name == 'KESassist09':
                innerModel_dataset_path = f'{get_raw_data_path()}/ASSISTments2009/assist09.npz'
        ...
        if self.args['load_model_test']:
            if self.args['agent'] == 'PFD':
                ...
                if os.path.exists(f'{get_proj_path()}/EduSim/SavedAgents/{self.agent.name}_{base_p_name}_{self.env.env_name}_{self.args["max_steps"]}_{self.args["max_episode_num"]}_{self.args["target_type"]}.pth'):
                    checkpoint = torch.load(f'{get_proj_path()}/EduSim/SavedAgents/{self.agent.name}_{base_p_name}_{self.env.env_name}_{self.args["max_steps"]}_{self.args["max_episode_num"]}_{self.args["target_type"]}.pth')
                else:
    ...
    def step(self):
        ...
        if self.agent.name == 'dktrec':
            agent_step = self.agent.step(input_data, candidates, len(self.state_t))
        else:
    ...
    def observe(self, observation, reward, done, info):
        ...
        if (self.agent.name == 'PFD' and self.args['PFD_privileged_reward']):




            b = self.env._exam(self.env._learner)
            ini = self.env._initial_score
            full = len(list(self.env._learner.target))
            reward = (b - ini) / (full - ini)
        ...
        if self.agent.name == 'PFD':
            input_data = get_feature_matrix(self.state_t,
                                            ...
                                            graph_embeddings=self.env.graph_embeddings)
            input_data = input_data.unsqueeze(0)
            DKT_states = torch.sigmoid(self.DKTnet(input_data))[max(0, len(self.state_t) - 1), 0, :]
            next_state_without_goal = self.rnnEncoder(input_data)[max(0, len(self.state_t) - 1), :, :]
    ...
    def end_episode(self, observation, reward, done, info):
        ...
        if not self.random_policy and not self.goal_nav_random:
            if not (self.agent.name == 'PFD' and self.args['PFD_privileged_reward']):
        ...
        if self.train_buffer.size() * self.max_sequence_length > self.begin_train_length and \
            ...
            for j in range(self.epoch_num):
                if self.dqn_advance_types['prioritized dqn'] and self.agent.name == 'dqn':
                    b_idx, b_memory, ISweights = self.train_buffer.sample(self.batch_size)
                    b_s = tuple(log[0] for log in b_memory)
                    b_a = tuple(log[1] for log in b_memory)
                    b_ns = tuple(log[3] for log in b_memory)
                    b_r = tuple(log[2] for log in b_memory)
                    b_d = tuple(log[4] for log in b_memory)
                    transition_dict = {'states': b_s, 'actions': b_a, 'next_states': b_ns, 'rewards': b_r, 'dones': b_d,
                                       'tree_idxs': b_idx,
                                       'ISweights': ISweights}
                else:
                    if self.agent.name == 'dqn':
        ...
        if self.args['episode_count'] == int(self.args['max_episode_num']) and not self.args['load_model_test']:
            if self.args['agent'] == 'PFD':
                ...
                torch.save({
                    ...
                    'args': self.args,
                }, f"{get_proj_path()}/EduSim/SavedAgents/{self.agent.name}_{base_p_name}_{self.env.env_name}_{self.args['max_steps']}_{self.args['max_episode_num']}_{self.args['target_type']}.pth")
            else:
                torch.save({
                    'model_state_dict': self.agent.state_dict(),
                    'args': self.args,
    ...
    def update(self, transition_dict):
        ...
        self.agg_model_optimizer.zero_grad()
    ...
    def model_based_roll_update(self):
        if self.args['agent'] == 'PFD':
            self.env._learner = self.initial_learner
            self.args['step_count'] = 0
            self.args['steptime_state_saver'] = torch.tensor([], dtype=torch.float).to(self.device)
            self.args['steptime_knowledge_state'] = torch.tensor([], dtype=torch.float).to(self.device)
            self.args['steptime_next_state_saver'] = torch.tensor([], dtype=torch.float).to(self.device)
