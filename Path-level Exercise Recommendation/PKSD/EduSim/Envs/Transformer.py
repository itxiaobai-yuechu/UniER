...
class PositionalEncoding(nn.Module):
    def __init__(self, embedding_size, dropout, max_len=20):
        ...
        self.dropout = nn.Dropout(dropout)
        positional = np.zeros((1, max_len, embedding_size))
        X = np.arange(0, max_len).reshape(-1, 1) / np.power(10000, np.arange(0, embedding_size, 2) / embedding_size)
        positional[:, :, 0::2] = np.sin(X)
        positional[:, :, 1::2] = np.cos(X)
        self.register_buffer('P', torch.tensor(positional, dtype=torch.float32), persistent=False)
