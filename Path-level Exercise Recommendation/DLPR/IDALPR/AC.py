...
class ActorCritic:
    ...
    def take_action(self, know, state, init_diff=None):
        if init_diff is not None:
            ...
            candidate_probs = probs.gather(dim=1, index=torch.tensor(candidates, dtype=torch.long, device=probs.device).view(1, -1))
            ques = int(random.choices(candidates, weights=candidate_probs.view(-1).detach().cpu().numpy())[0])
        return ques
