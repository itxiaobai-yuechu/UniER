...
class DIMKT(Module):
    ...
    def forward(self,q,c,sd,qd,a,qshft,cshft,sdshft,qdshft):
        ...
        padd = torch.zeros(shape[0],1,shape[2],device=sd_emb.device)
        sd_emb = cat((padd,sd_emb),1)
        slice_sd_embedding = sd_emb.split(1,dim=1)

        shape = list(a_emb.shape)
        padd = torch.zeros(shape[0],1,shape[2],device=a_emb.device)
        a_emb = cat((padd,a_emb),1)
        slice_a_embedding = a_emb.split(1,dim=1)

        shape = list(input_data.shape)
        padd = torch.zeros(shape[0],1,shape[2],device=input_data.device)
        ...
        k = self.knowledge.repeat(self.batch_size, 1).to(input_data.device)
