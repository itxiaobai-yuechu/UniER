...
class Learner(MetaLearner):
    def __init__(self,
        ...
        self._logs_q = initial_log_q
        self._state = []
        self.random_state = np.random.RandomState(seed)

    def update_logs(self, logs, logs_q):
        self._logs = logs
        self._logs_q = logs_q
    ...
    def learn(self, learning_item, question_item, score):
        self._logs.append([learning_item, score])
        self._logs_q.append([question_item, score])
...
class LearnerGroup(MetaInfinityLearnerGroup):
    def __init__(self, dataRec_path, dataRec_q_path, seed=None):
        ...
        self.data_q_path = dataRec_q_path
        self.random_state = np.random.RandomState(seed)
        if not os.path.isdir(self.data_path) and not 'npz' in self.data_path:
            with open(self.data_path, 'r', encoding="utf-8") as f:
                self.datatxt = f.readlines()
        if not os.path.isdir(self.data_q_path) and not 'npz' in self.data_q_path:
            with open(self.data_q_path, 'r', encoding="utf-8") as f:
                self.datatxt_q = f.readlines()
    ...
    def __next__(self):
        ...
        while len({log[0] for log in session}) < 20:
            ...
            session_q = json.loads(self.datatxt_q[index])
        ...
        initial_log_q = copy.deepcopy(session_q[:int(len(session_q) * 0.6)])
