...
import rl_utils
import random
from AdaptiveCN import Adaptive_Cognitive_Nevigation,Find_Similarity_Neighbors
import collections
...
class PPO_sim:
    ...
    def get_torlerance(self,state,last_tor,last_prac_num,diff,threshhold):
        ...
        if os.path.exists(tol_model_path):
            ...
            torlerance = 3 + (2 * (delta / ability) if ability != 0 else (last_tor+5))
            if torlerance > 20:
                torlerance = 20
    ...
    def take_action(self, state, last_item, target, Know_G,k_hop,threshhold, last_tor=3,last_prac_num=2, explore=False):
        ...
        candidates = Find_Similarity_Neighbors(Know_G,target,state,threshhold,k_hop)
        if explore:
            candidate_probs = probs.gather(dim=1, index=torch.Tensor(candidates).long().to(probs.device).view(1, -1))
        else:
            candidate_probs = probs.gather(dim=1, index=torch.Tensor(candidates).long().to(probs.device).view(1, -1))
        action = int(random.choices(candidates, weights=candidate_probs.view(-1).detach().cpu().numpy())[0])
