...
class MyDataset(Dataset):
    ...
    def get_selected_data(self):
        if self.data_path != f'{get_proj_path()}/data/dataProcess/{self.dataset}/student_log_kt_None':
            raise ValueError('This function can only be used under env KT training situation')

        env_selected_train_data_path = f'{get_proj_path()}/data/dataProcess/{self.dataset}/student_log_kt_None_selected'
        selecte_count = 0
...
class EnvKTtrainer:
    def __init__(self, dataset, dataset_KES, train_goal='env_KT', device=None):
        ...
        self.device = device
        with open(f'{get_proj_path()}/data/dataProcess/{self.dataset}/graph_vertex.json') as f:
            ku_dict = json.load(f)
        ...
        if self.env_selected_data_train:
            self.env_KT_data_path = f'{get_proj_path()}/data/dataProcess/{self.dataset}/student_log_kt_None_selected'
            print(self.env_KT_data_path)
        else:
            self.env_KT_data_path = f'{get_proj_path()}/data/dataProcess/{self.dataset}/student_log_kt_None'
        self.agent_KT_data_path = f'{get_proj_path()}/data/dataProcess/{self.dataset}/dataOff'
